...
{
	// Weapon Type
	public EasyWeapons.WeaponType type = EasyWeapons.WeaponType.Projectile;		// Which weapon system should be used
	...
	// EasyWeapons.Auto
	public EasyWeapons.Auto auto = EasyWeapons.Auto.Full;						// How does this weapon fire - semi-auto or full-auto
	...
	public EasyWeapons.BulletHoleSystem bhSystem = EasyWeapons.BulletHoleSystem.Tag;	// What condition the dynamic bullet holes should be based off
	...
	void Update()
	{
		...
		// Recoil Recovery
		if (playerWeapon && recoil && type != EasyWeapons.WeaponType.Beam)
		{
		...
		if (type == EasyWeapons.WeaponType.Beam)
		{
			if (!beaming)
				StopBeam();
	...
	void CheckForUserInput()
	{
		...
		if (type == EasyWeapons.WeaponType.Raycast)
		{
		...
		if (type == EasyWeapons.WeaponType.Projectile)
		{
			if (fireTimer >= actualROF && burstCounter < burstRate && canFire)
			{
				if (Input.GetButton("Fire1"))
	...
	void OnGUI()
	{
		...
		// Crosshairs
		if (type == EasyWeapons.WeaponType.Projectile || type == EasyWeapons.WeaponType.Beam)
		{
		...
		if (showCurrentAmmo)
		{
			if (type == EasyWeapons.WeaponType.Raycast || type == EasyWeapons.WeaponType.Projectile)
				GUI.Label(new Rect(10, Screen.height - 30, 100, 20), "Ammo: " + currentAmmo);
			else if (type == EasyWeapons.WeaponType.Beam)
	...
	void Fire()
	{
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				if (bhSystem == EasyWeapons.BulletHoleSystem.Tag)
				{
				...
				else if (bhSystem == EasyWeapons.BulletHoleSystem.Material)
				{
					foreach (SmartBulletHoleGroup bhg in bulletHoleExceptions)
					{
				...
				else if (bhSystem == EasyWeapons.BulletHoleSystem.Physic_Material)
				{
					foreach (SmartBulletHoleGroup bhg in bulletHoleExceptions)
					{
